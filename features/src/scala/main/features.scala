...
object Features {
  def main(args: Array[String]) {
    ...
    /** join processed_matches with the summoner_ids of that match (on match_id)
     * and generate pos of the player **/
    val participants = processed_matches.join(matches, $"p_match_id" === $"matchId").drop("matchId")
      .map(x => {
      def generate_pos (pos: Any, role: Any):Int = {
            ...
            def fill_pos(
              matchInfo: (Int, String, String, Boolean, Long),
              participant_info: WrappedArray[(Int, String, String, Boolean, Long)]
              ) = {
    ...
    val participants_flat = participants
      .explode("summs_infos", "summ_info"){x:WrappedArray[(Integer, String, String, Boolean)] => x}
      .drop("summs_infos").map(x => {
      val participant_info = x(1) match {
        case p: Row => (
          p(0).asInstanceOf[Integer],
          p(1).asInstanceOf[String],
          p(2).asInstanceOf[String],
          p(3).asInstanceOf[Boolean])
      }
      (
    ...
    def generateFeatures(key:Row, rows:Iterator[ParticipantMatch]):List[Ret] = {
      ...
      val matchId = key(0).asInstanceOf[String]
        ...
        val features = participants_matches.
        groupBy($"p_match_id", $"summ_id", $"summ_pos", $"team", $"winner")
          .flatMapGroups(generateFeatures).toDF()
    ...
    val out_features = features.groupBy(Seq("winningTeam", "matchId").map(col): _*)
      .pivot("playerNum", (1 to 10).toList).agg(exprs.head, exprs.tail: _*)

    val out_players = features.groupBy(Seq("summId").map(col): _*).agg(exprs.head, exprs.tail: _*)


    /** save the features **/
    out_features.coalesce(1).write.mode(SaveMode.Overwrite)
    .format("com.databricks.spark.csv").save(outputPath + "out_features.csv")

    out_players.coalesce(1).write.mode(SaveMode.Overwrite)
    .format("com.databricks.spark.csv").save(outputPath + "out_players.csv")
